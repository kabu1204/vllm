...
try:
    ...
    from vllm_cpu_ops import ops as cpu_ops
    from vllm_cpu_ops import cache_ops as cpu_cache_ops
except ImportError:
    pass
...
def gelu_tanh_and_mul(out: torch.Tensor, x: torch.Tensor) -> None:
    if x.is_cpu:
        cpu_ops.gelu_tanh_and_mul(out, x)
    else:
...
def paged_attention_v1(
    ...
    if query.is_cpu:
        cpu_ops.paged_attention_v1(out, query, key_cache, value_cache,
...
def paged_attention_v2(
    ...
    if query.is_cpu:
        cpu_ops.paged_attention_v2(out, exp_sum, max_logits, tmp_out, query,
...
def rotary_embedding(
    ...
    if query.is_cpu:
        cpu_ops.rotary_embedding(positions, query, key, head_size, cos_sin_cache,
                              is_neox)
    else:
...
def batched_rotary_embedding(positions: torch.Tensor, query: torch.Tensor,
    ...
    if query.is_cpu:
        cpu_ops.batched_rotary_embedding(positions, query, key, head_size,
...
def rms_norm(out: torch.Tensor, input: torch.Tensor, weight: torch.Tensor,
             epsilon: float) -> None:
    if input.is_cpu:
        cpu_ops.rms_norm(out, input, weight, epsilon)
    else:
...
def fused_add_rms_norm(input: torch.Tensor, residual: torch.Tensor,
    ...
    if input.is_cpu:
        cpu_ops.fused_add_rms_norm(input, residual, weight, epsilon)
    else:
...
def awq_dequantize(qweight: torch.Tensor, scales: torch.Tensor,
    ...
    if qweight.is_cpu:
        return cpu_ops.awq_dequantize(qweight, scales, zeros, split_k_iters, thx,
                                   thy)
    else:
...
def awq_gemm(input: torch.Tensor, qweight: torch.Tensor, qzeros: torch.Tensor,
    ...
    if input.is_cpu:
        return cpu_ops.awq_gemm(input, qweight, qzeros, scales, split_k_iters)
    else:
...
def gptq_gemm(a: torch.Tensor, b_q_weight: torch.Tensor,
    ...
    if a.is_cpu:
        return cpu_ops.gptq_gemm(a, b_q_weight, b_gptq_qzeros, b_gptq_scales,
...
def gptq_shuffle(q_weight: torch.Tensor, q_perm: torch.Tensor,
                 bit: int) -> None:
    if q_weight.is_cpu:
        cpu_ops.gptq_shuffle(q_weight, q_perm, bit)
    else:
...
def squeezellm_gemm(vec: torch.Tensor, mat: torch.Tensor, mul: torch.Tensor,
    ...
    if vec.is_cpu:
        cpu_ops.squeezellm_gemm(vec, mat, mul, lookup_table)
    else:
...
def marlin_gemm(a: torch.Tensor, b_q_weight: torch.Tensor,
    ...
    if a.is_cpu:
        return cpu_ops.marlin_gemm(a, b_q_weight, b_scales, workspace, size_m,
                                size_n, size_k)
    else:
...
def gptq_marlin_24_gemm(a: torch.Tensor, b_q_weight: torch.Tensor,
                        ...
                        size_n: int, size_k: int) -> torch.Tensor:
    if a.is_cpu:
        return cpu_ops.gptq_marlin_24_gemm(a, b_q_weight, b_meta, b_scales,
...
def cutlass_scaled_mm_dq(a: torch.Tensor, b: torch.Tensor,
    ...
    if a.is_cpu:
        cpu_ops.cutlass_scaled_mm_dq(out, a, b, a_scales, b_scales)
    else:
...
def aqlm_gemm(input: torch.Tensor, codes: torch.Tensor,
    ...
    if input.is_cpu:
        return cpu_ops.aqlm_gemm(input, codes, codebooks, scales,
...
def aqlm_dequant(codes: torch.Tensor, codebooks: torch.Tensor,
    ...
    if codes.is_cpu:
        return cpu_ops.aqlm_dequant(codes, codebooks, codebook_partition_sizes)
    else:
...
def gptq_marlin_repack(b_q_weight: torch.Tensor, perm: torch.Tensor,
    ...
    if b_q_weight.is_cpu:
        return cpu_ops.gptq_marlin_repack(b_q_weight, perm, size_k, size_n,
                                       num_bits)
    else:
...
def gptq_marlin_gemm(a: torch.Tensor, b_q_weight: torch.Tensor,
    ...
    if a.is_cpu:
        return cpu_ops.gptq_marlin_gemm(a, b_q_weight, b_scales, g_idx, perm,
                                     workspace, num_bits, size_m, size_n,
                                     size_k, is_k_full)
    else:
...
def scaled_fp8_quant(
    ...
    if scale is None:
        ...
        if input.is_cpu:
            cpu_ops.dynamic_scaled_fp8_quant(output, input, scale)
        else:
            vllm_ops.dynamic_scaled_fp8_quant(output, input, scale)
    else:
        if input.is_cpu:
            cpu_ops.static_scaled_fp8_quant(output, input, scale)
        else:
...
def moe_align_block_size(topk_ids: torch.Tensor, num_experts: int,
    ...
    if topk_ids.is_cpu:
        cpu_ops.moe_align_block_size(topk_ids, num_experts, block_size,
...
def reshape_and_cache_flash(
    ...
    if key.is_cpu:
        cpu_cache_ops.reshape_and_cache_flash(key, value, key_cache, value_cache,
...
def copy_blocks(key_caches: torch.Tensor, value_caches: torch.Tensor,
    ...
    if key_caches.is_cpu:
        cpu_cache_ops.copy_blocks(key_caches, value_caches, block_mapping)
    else:
...
def swap_blocks(src: torch.Tensor, dst: torch.Tensor,
                block_mapping: torch.Tensor) -> None:
    print(f"src: {src.device}, dst: {dst.device}")
...
def convert_fp8(output: torch.Tensor,
    ...
    if input.is_cpu:
        cpu_cache_ops.convert_fp8(output, input, scale, kv_dtype)
    else:
