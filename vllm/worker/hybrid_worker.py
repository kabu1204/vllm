...
class HybridWorker(WorkerBase):
    ...
    Each worker is associated with a single GPU or a CPU core group. 
    The worker is responsible for maintaining the KV cache and executing the model. 
    In case of distributed inference, each worker is assigned a partition of the model.
    """
    ...
    def __init__(
        ...
        self.hybrid_world_size = parallel_config.world_size + parallel_config.num_cpu_workers
    ...
    def init_device(self) -> None:
        assert self.parallel_config.distributed_executor_backend == "hybrid", "HybridWorker only support \'hybrid\' device type"
        if self.is_cpu_worker:
            self.device = torch.device("cpu")
            num_cpu_threads = torch.get_num_threads()//self.parallel_config.num_cpu_workers
        ...
        init_hybrid_worker_distributed_environment(self.parallel_config, self.rank,
...
def init_hybrid_worker_distributed_environment(
    parallel_config: ParallelConfig,
    rank: int,
