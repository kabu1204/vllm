...
class HybridModelRunner:
    ...
    def debug_split_kv_cache(
        ...
        num_kv_heads = self.model_config.hf_config.num_attention_heads
        head_size = self.model_config.hf_config.hidden_size // num_kv_heads
        if kv_cache is None:
            return None, None
        x = 16 // kv_cache.element_size()
        print(f"first_block = {x}")
        ...
        t = key_cache[23878]
        tmp = torch.zeros(num_kv_heads, head_size//x, 16, x, device="cpu", dtype=torch.float16)
        tmp.copy_(t)
        torch.save(tmp, "1.pt")
    ...
    def execute_model(
        ...
        # self.debug_split_kv_cache(kv_caches[0], 0, 0)
